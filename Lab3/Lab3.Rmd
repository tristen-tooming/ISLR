...
fit_ns_15 = lm(lmr ~ ns(Day, df = 16), data = mortality_rate)
pred3 = predict(fit_ns_15, newdata = list(Day=mortality_rate$Day))

fit_ns_50 = lm(lmr ~ ns(Day, df = 51), data = mortality_rate)
pred4 = predict(fit_ns_50, newdata = list(Day=mortality_rate$Day))


plot(mortality_rate$Day, mortality_rate$lmr, xlab = 'Day', ylab = 'LMR', pch=16,
     main = 'Natural cubic splies fit')
lines(mortality_rate$Day, pred1, col='black', lwd = 2)
points(attr(ns(mortality_rate$Day, df = 2), "knots"),
  predict(fit_ns_1, 
  data.frame(x = attr(ns(x, df = 2), 
  "knots"))),
  col="black",pch=15)
...
legend(1, 0, legend=c(1, 2, 15, 50),
       col=c('black', 'blue', 'green', 'red'), lwd = 2, cex=0.8)
```
